local RadiationAura = Class(function(self, inst)
    self.inst = inst
    self.aura = 0
	...
	self.inst:AddTag("radiationaura")
end)

function RadiationAura:OnRemoveFromEntity()
	self.inst:RemoveTag("radiationaura")
end

function RadiationAura:GetAura(observer)
	local aura_val = 0
