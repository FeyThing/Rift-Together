    ...
    State{
        ...
        onenter = function(inst)
            ...
            elseif inst:HasTag("weremoose") then
                ...
                inst.AnimState:PlayAnimation("punch")
				inst.AnimState:SetSymbolBloom("annihilator_blade")
				inst.AnimState:SetSymbolLightOverride("annihilator_blade", .5)
				inst.AnimState:SetLightOverride(.1)		
				inst.sg.statemem.annihilator = true
                inst.SoundEmitter:PlaySound("dontstarve/wilson/attack_weapon", nil, nil, true)				
                cooldown = math.max(cooldown, 13 * FRAMES)
            end
        ...
        {			
			TimeEvent(5 * FRAMES, function(inst)
                if inst.sg.statemem.annihilator then
                    inst.SoundEmitter:PlaySound("WX_rework/shock/big", nil, nil, true)
					local x, y, z = inst.Transform:GetWorldPosition()
						local rot = inst.Transform:GetRotation()

						--V2C: first frame is blank, so no need to worry about forcing instant facing update
						local fx = SpawnPrefab("sparks") --hitsparks_fx
...
return StateGraph("nefarious", states, events, "idle", actionhandlers)
