...
local TARGET_IGNORE_TAGS = { "INLIMBO", "dustdevilproof" }
local TARGET_IGNORE_TAGS_2 = { "INLIMBO", "goggles" }

local function destroystuff(inst, forcelanded)
        ...
        if v ~= inst.WINDSTAFF_CASTER and v:IsValid() then
            if v.components.health ~= nil and
                ...
                local damage =
                    ...
                    v:HasTag("player") and
                    TUNING.DUSTDEVIL_DAMAGE * TUNING.PVP_DAMAGE_MOD or
                    TUNING.DUSTDEVIL_DAMAGE
                v.components.combat:GetAttacked(inst, damage, nil, "wind")
				v:PushEvent("knockback", { knocker = inst, radius = 2, strengthmult = 1, forcelanded = forcelanded })
                if v:IsValid() and
...
--[[local function sandover(inst)
	
    local x, y, z = inst.Transform:GetWorldPosition()
    local ents = TheSim:FindEntities(x, y, z, 7, nil, TARGET_IGNORE_TAGS_2, TARGET_TAGS)
    for i, v in ipairs(ents) do
		function inst:CreateOverlays(ents)
		fn(self, ents)
		self.sandover = self.overlayroot:AddChild(SandOver(ents))
	end
    end
end]]
...
return StateGraph("dustdevil", states, events, "empty")
