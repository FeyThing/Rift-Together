local assets = {
    Asset("ANIM", "anim/luminoustree_build.zip"),
    Asset("ANIM", "anim/luminoustree_normal.zip"),
    Asset("ANIM", "anim/luminoustree_short.zip"),
    Asset("ANIM", "anim/luminoustree_tall.zip"),
    --Asset("MINIMAP_IMAGE", "luminoustree"),
    --Asset("MINIMAP_IMAGE", "luminoustree_burnt"),
    --Asset("MINIMAP_IMAGE", "luminoustree_stump"),
}
...
local luminoustree_anims = {
    [SHORT] = makeanims(SHORT),
    [TALL] = makeanims(TALL),
    [NORMAL] = makeanims(NORMAL),
}

SetSharedLootTable("luminoustree_small",
{
    {"log", 1.0},
    {"log", 1.0},
...
SetSharedLootTable("luminoustree_normal",
{
    {"log", 1.0},
    {"log", 1.0},
    {"log", 1.0},
...
SetSharedLootTable("luminoustree_tall",
{
    {"log", 1.0},
    {"log", 1.0},
    {"log", 1.0},
...
SetSharedLootTable("luminoustree_burnt",
{
    {"charcoal", 1.0},
})

-- Function for chopping down a moon tree that has been burnt.
local function chop_down_burnt_luminoustree(inst, chopper)
    ...
    inst.AnimState:PlayAnimation(luminoustree_anims[inst.size].chop_burnt)
...
local function burnt_changes(inst)
    ...
    inst.components.lootdropper:SetChanceLootTable("luminoustree_burnt")

    if inst.components.workable then
        inst.components.workable:SetWorkLeft(1)
        inst.components.workable:SetOnWorkCallback(nil)
        inst.components.workable:SetOnFinishCallback(chop_down_burnt_luminoustree)
    end
end

local function on_luminoustree_burnt_immediate_helper(inst, immediate)
    if immediate then
        burnt_changes(inst)
    else
    ...
    inst.AnimState:PlayAnimation(luminoustree_anims[inst.size].burnt, true)
    inst.MiniMapEntity:SetIcon("luminoustree_burnt.png")
...
local function on_luminoustree_burnt(inst)
    on_luminoustree_burnt_immediate_helper(inst, false)
end

local function inspect_luminoustree(inst)
...
local function on_chop_tree(inst, chopper, chops_remaining, num_chops)
    ...
    local anim_set = luminoustree_anims[inst.size]
...
local function make_stump(inst)
    ...
    inst.MiniMapEntity:SetIcon("luminoustree_stump.png")
...
local function sway(inst)
    local anim_to_play = (math.random() > .5 and luminoustree_anims[inst.size].sway1) or luminoustree_anims[inst.size].sway2
...
local function set_short_burnable(inst)
    ...
    inst.components.burnable:SetOnBurntFn(on_luminoustree_burnt)
...
local function set_short(inst)
    ...
    inst.components.lootdropper:SetChanceLootTable("luminoustree_small")
    inst:AddTag("shelter")
    sway(inst)
end
...
local function set_normal(inst)
    ...
    inst.components.lootdropper:SetChanceLootTable("luminoustree_normal")
    inst:AddTag("shelter")
    sway(inst)
end
...
local function set_tall(inst)
    ...
    inst.components.lootdropper:SetChanceLootTable("luminoustree_tall")
    inst:AddTag("shelter")
    sway(inst)
end
...
local function on_load(inst, data)
    ...
    elseif is_burnt then
        on_luminoustree_burnt_immediate_helper(inst, true)
    else
        sway(inst)
    end
end
...
local function on_wake(inst)
    ...
    if inst.components.inspectable == nil then
        ...
        inst.components.inspectable.getstatus = inspect_luminoustree
    end
end
...
local function luminoustree(name, stage, data)
    local function fn()
        ...
        inst.MiniMapEntity:SetIcon("luminoustree.tex")
        ...
        --inst.components.plantregrowth:SetProduct("luminoustree_sapling")
        --inst.components.plantregrowth:SetSearchTag("luminoustree")
