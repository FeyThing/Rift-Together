local function onupdate(inst, dt)
    --[[if inst.sound then
		...
		inst.SoundEmitter:SetVolume("dontstarve/characters/wx78/spark", .1)
        inst.sound = nil
    end]]
...
local function StartFX(proxy, animindex, build)
    ...
    inst.AnimState:SetBank("radiation_clouds")
    inst.AnimState:SetBuild("radiation_clouds")
...
local function MakeRadiation_Clouds(name, build)
    local assets =
    {
        Asset("ANIM", "anim/radiation_clouds.zip"),
    }
...
return MakeRadiation_Clouds("radiation_clouds", "radiation_clouds")
